...
def main(args):
    ...
    progress = {k: [] for k in ['epoch', 'duration',
                                'train_loss', 'train_entropy_loss', 'train_adjacency_loss', 'train_acc',
                                'val_loss', 'val_entropy_loss', 'val_adjacency_loss', 'val_acc']}
    ...
    for epoch in range(args.num_epochs):
        ...
        train_loss = train_adjacency_loss + train_entropy_loss
        ...
        with th.no_grad():
            ...
            val_loss = val_entropy_loss + val_adjacency_loss
        ...
        print("Epoch {:05d} | Time(s) {:.4f} | Train Loss {:.4f} | Train CE Loss {:.4f} | Train Graph Loss {:.4f} | Train Acc {:.4f} |"
              " Val Loss {:.4f} | Val CE Loss {:.4f} | Val Graph Loss {:.4f} | Val Acc {:.4f}".format(
                  epoch, np.mean(dur),
                  train_loss.item(), train_entropy_loss.item(), train_adjacency_loss.item(), train_acc,
                  val_loss.item(), val_entropy_loss.item(), val_adjacency_loss.item(), val_acc))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nu',
                        type=float,
                        default=10,
                        help='Regularizing weight on custom loss.')
