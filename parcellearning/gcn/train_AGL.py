...
def main(args):
    ...
    train_weight = th.matmul(hot_encoding.t(), th.matmul(training.adjacency_matrix(), hot_encoding))
    train_weight = (train_weight>0).float()

    validation_list = '%svalidation.txt' % (data_dir)
    validation = gnnio.loaddata(
        args.features, validation_list, dType='validation')
    ...
    progress = {k: [] for k in ['epoch', 'duration',
                                'train_loss', 'train_entropy_loss', 'train_graph_loss', 'train_adjacency_loss', 'train_acc',
                                'val_loss', 'val_entropy_loss', 'val_graph_loss', 'val_adjacency_loss', 'val_acc']}
    ...
    adjacency_loss = losses.AdjacentLabelLoss()
    ...
    for epoch in range(args.num_epochs):
        ...
        # graph loss enforcing embedding distances
        train_graph_loss = args.mu * graph_loss(training, train_sf)
        # adjacency loss penalizing when labels that arent adjacent in training set are adjacent in prediction
        train_adjacency_loss = args.nu * adjacency_loss(training, train_sf, train_indices, train_weight)
        
        train_loss = train_entropy_loss + train_adjacency_loss + train_graph_loss
        ...
        with th.no_grad():
            ...
            # graph loss enforcing embedding distances
            val_graph_loss = args.mu * graph_loss(validation, val_sf)
            # adjacency loss penalizing when labels that arent adjacent in training set are adjacent in prediction
            val_adjacency_loss = args.nu * adjacency_loss(validation, val_sf, val_indices, train_weight)

            val_loss = val_entropy_loss + val_adjacency_loss + val_graph_loss
        ...
        print("Epoch {:05d} | Time(s) {:.4f} |"
              " Train Loss {:.4f} | Train Entropy Loss {:.4f} | Train Graph Loss {:.4f} | Train Adjacency Loss {:.4f} | Train Acc {:.4f} |"
              " Val Loss {:.4f} | Val Entropy Loss {:.4f} | Val Graph Loss {:.4f} | Val Adjacency Loss {:.4f} | Val Acc {:.4f} |".format(
                  epoch, np.mean(dur),
                  train_loss.item(), train_entropy_loss.item(), train_graph_loss.item(), train_adjacency_loss.item(), train_acc,
                  val_loss.item(), val_entropy_loss.item(), val_graph_loss.item(), val_adjacency_loss.item(), val_acc))
        ...
        progress['train_adjacency_loss'].append(train_adjacency_loss.item())
        ...
        progress['val_adjacency_loss'].append(val_adjacency_loss.item())
    ...
    with open(performance_output, "w") as outparams:
        json.dump(progress, outparams, ensure_ascii=True,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--margin',
                        type=float,
                        default=0.1,
                        help='Slack variable controlling margin between negative and positive samples in graph loss.')
    ...
    parser.add_argument('--nu',
                        type=float,
                        default=10,
                        help='Regularizing weight on adjacency loss.')
